...
const Clinics = () => {
    ...
    const [activeParentCategory, setActiveParentCategory] = useState(() => {
        try { return JSON.parse(localStorage.getItem("activeCategory")); } catch { return null; }
    });

    useEffect(() => {
        const handleStorageChange = () => {
            try { setActiveParentCategory(JSON.parse(localStorage.getItem("activeCategory"))); } catch { setActiveParentCategory(null); }
        };
    ...
    const initialForm = {
        name: "", districtId: "", managerId: "", address: "", hotline: "", email: "", description: "",
        mapsUrl: "", longitude: "", latitude: "", openTime: "07:30", closeTime: "19:30", availableServiceIds: [], category: "",
    };
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: (res, deletedId) => {
            ...
            setIsDeleteModalOpen(false); setClinicToDelete(null);
            queryClient.invalidateQueries({ queryKey: ["clinics"] });
        },
        onError: (err) => showToast(translateErrorMessage(err.response?.data?.message || err.message) || "Không thể xóa do đang có dữ liệu liên kết", "error")
    });
    ...
    const saveMutation = useMutation({
                ...
                if (variables.id) {
                    return old.map(c => {
                        if (c._id === variables.id) {
                            return {
                                ...c, name: formData.name, address: formData.address, hotline: formData.hotline,
                                districtId: selectedDistrict, imageUrls: imagePreviews.length > 0 ? imagePreviews : oldImageUrls
                            };
                        }
                        return c;
                    });
                    ...
                    const newClinic = {
                        _id: savedClinic?._id || Date.now().toString(), name: formData.name, address: formData.address,
                        hotline: formData.hotline, districtId: selectedDistrict, isActive: true, imageUrls: imagePreviews,
                        totalRating: 0, totalReview: 0, ...savedClinic
                    };
        ...
        onError: (err) => showToast(translateErrorMessage(err.response?.data?.message || err.message) || "Có lỗi xảy ra khi lưu phòng khám", "error")
    });
    ...
    const openAddModal = () => {
        setIsEditMode(false); setEditClinicId(null);
    ...
    const openEditModal = (e, clinic) => {
        e.stopPropagation();
        setIsEditMode(true); setEditClinicId(clinic._id);
        ...
        setFormData({
            name: clinic.name || "", districtId: clinic.districtId?._id || clinic.districtId || "",
            address: clinic.address || "", hotline: clinic.hotline || "",
            managerId: clinic.managerId?._id || clinic.managerId || "", email: clinic.email || "",
            description: clinic.description || "", mapsUrl: clinic.mapsUrl || "",
            longitude: lng, latitude: lat, openTime: clinic.openingHours?.openTime || "07:30",
            closeTime: clinic.openingHours?.closeTime || "19:30", availableServiceIds: clinicServiceIds,
    ...
    const handleServiceCheckbox = (serviceId) => setFormData((prev) => ({
        ...prev,
    ...
    const handleSelectAllServices = () => setFormData((prev) => ({
        ...prev, availableServiceIds: isAllServicesSelected ? [] : services.map((s) => s._id)
    }));
    ...
    const removeOldImage = (index) => {
        const updated = [...oldImageUrls]; updated.splice(index, 1);
    ...
    const adminOptions = admins.map((admin) => ({ value: admin.userId || admin._id, label: `${admin.fullName || admin.username || "Chưa có tên"} - ${admin.email || admin.phone || ""}` }));
        ...
        <>
            ...
            <div className="z-clinic-container">
                ...
                <div className="z-clinic-tools">
                    ...
                    <div className="z-clinic-filter">
                        ...
                        {showFilterDropdown && (
                            <div className="z-clinic-dropdown-menu">
                                <div className={`z-clinic-dropdown-item ${filterStatus === "all" ? "active" : ""}`} onClick={() => { setFilterStatus("all"); setShowFilterDropdown(false); }}>Tất cả trạng thái</div>
                                <div className={`z-clinic-dropdown-item ${filterStatus === "active" ? "active" : ""}`} onClick={() => { setFilterStatus("active"); setShowFilterDropdown(false); }}>Đang hoạt động</div>
                                <div className={`z-clinic-dropdown-item ${filterStatus === "inactive" ? "active" : ""}`} onClick={() => { setFilterStatus("inactive"); setShowFilterDropdown(false); }}>Ngừng hoạt động</div>
                            </div>
                        )}
                    </div>
                    ...
                    {isSuperAdmin && (
                        <AddButton onClick={openAddModal} className="z-clinic-add-btn-pull-right" style={{ marginLeft: "auto" }}>Thêm Phòng Khám</AddButton>
                    )}
                </div>
                    ...
                    <table className="z-clinic-table">
                        ...
                        <tbody>
                            {filteredClinics.length === 0 ? (
                                <tr><td colSpan="7"><div className="z-clinic-state">Không có dữ liệu phòng khám nào.</div></td></tr>
                            ) : (
                                filteredClinics.map((clinic, index) => {
                                    ...
                                    return (
                                        <tr key={clinic._id} onClick={() => handleRowClick(clinic._id)} className="z-clinic-clickable-row">
                                            <td className="z-clinic-td-center"><strong>{index + 1}</strong></td>
                                            <td>
                                                <img src={clinic.imageUrls?.[0] || FALLBACK_IMG} alt={clinic.name} className="z-clinic-table-img" onError={(e) => { e.target.src = FALLBACK_IMG; }} />
                                            </td>
                                            <td>
                                                <div className="z-clinic-text-bold">{clinic.name}</div>
                                                <div className="z-clinic-subtext">Hotline: {clinic.hotline}</div>
                                                {managerName ? (
                                                    <div className="z-clinic-subtext" style={{ marginTop: "4px", fontSize: "12px", color: "#4b5563" }}>Quản lý: <span style={{ fontWeight: "600", color: "var(--primary-color, #312e81)" }}>{managerName}</span></div>
                                                ) : (
                                                    <div className="z-clinic-subtext" style={{ marginTop: "4px", fontSize: "12px", fontStyle: "italic", color: "#9ca3af" }}>Chưa gán quản lý</div>
                                                )}
                                            </td>
                                            ...
                                            <td>
                                                <div className="z-clinic-dropdown-actions" onClick={(e) => e.stopPropagation()}>
                                                    ...
                                                    <div className="z-clinic-action-menu">
                                                        <Button variant="outline" onClick={(e) => { e.stopPropagation(); toggleStatusMutation.mutate(clinic._id); }} disabled={toggleStatusMutation.isPending}>{clinic.isActive ? "Tạm dừng" : "Kích hoạt"}</Button>
                        ...
                        <div className="z-clinic-form-grid">
                            <div className="z-clinic-form-column">
                                <div className="z-clinic-form-group">
                                    ...
                                    <input type="text" value={activeParentCategory?.title || "Chưa xác định"} disabled className="z-clinic-input readonly z-clinic-input-highlight" />
                                </div>
                                <div className="z-clinic-form-group">
                                ...
                                <div className="z-clinic-form-group">
                                    <label>Quản lý (Admin) <span className="z-clinic-required">*</span></label>
                            ...
                            <div className="z-clinic-form-column">
                                ...
                                <div className="z-clinic-form-group">
                                    <label>Link Google Maps <span className="z-clinic-required">*</span></label>
                                ...
                                <div className="z-clinic-form-row">
                                    <div className="z-clinic-form-group z-clinic-flex-1">
                                        <label>Kinh độ (Lng) <span className="z-clinic-required">*</span></label>
                                    ...
                                    <div className="z-clinic-form-group z-clinic-flex-1">
                                        <label>Vĩ độ (Lat) <span className="z-clinic-required">*</span></label>
                                ...
                                <div className="z-clinic-form-group">
                                    <label>Dịch vụ cung cấp <span className="z-clinic-required">*</span></label>
                                ...
                                <div className="z-clinic-form-group">
                                    ...
                                    <div className="z-clinic-upload-wrapper">
                                        ...
                                        {oldImageUrls.length + imagePreviews.length < 5 && (
                                            <div className="z-clinic-add-img-btn" onClick={handleAddImageClick}>+ Ảnh</div>
                                        )}
                                    </div>
                                </div>
                            </div>
                ...
                <Modal isOpen={isDeleteModalOpen} onClose={() => !isSubmitting && setIsDeleteModalOpen(false)} title="Xác nhận xóa" maxWidth="700px" onSave={() => deleteMutation.mutate(clinicToDelete?.id)} saveText={isSubmitting ? "Đang xóa..." : "Xác nhận xóa"}>
                    <div className="z-clinic-delete-content">
                        <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" viewBox="0 0 24 24" fill="none" stroke="#eb3c2f" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6h18"></path><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"></path><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"></path></svg>
                        <h3 className="z-clinic-delete-title">Xóa phòng khám?</h3>
                        <p className="z-clinic-delete-text">Bạn có chắc muốn xóa phòng khám <br /> <strong>{clinicToDelete?.name}</strong> không?</p>
                    </div>
                </Modal>
            </div>
        </>
